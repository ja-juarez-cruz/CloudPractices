...
# Habilitar versionamiento para proteger contra eliminaciones accidentales
...
# Encriptación server-side con AES-256
...
# Políticas de lifecycle para gestión automática de costos
resource "aws_s3_bucket_lifecycle_configuration" "backup_lifecycle" {
  bucket = aws_s3_bucket.backup_bucket.id

  # Regla 1: Transición a Glacier para backups antiguos
  rule {
    ...
    # Mover a Glacier después del período de retención
    transition {
      days          = var.retention_days
      storage_class = "GLACIER"
    }

    # Mover versiones antiguas a Glacier más rápido
    noncurrent_version_transition {
      noncurrent_days = 30
      storage_class   = "GLACIER"
    }

    # Eliminar versiones muy antiguas
    noncurrent_version_expiration {
      noncurrent_days = 365
    }
  }

  # Regla 2: Expiración de backups muy antiguos (1 años)
  rule {
  ...
  # Regla 3: Limpiar uploads incompletos
  rule {
...
# Policy para permitir que Lambda publique
...
data "archive_file" "backup_lambda_zip" {
  type        = "zip"
  source_file = "${path.module}/src/lambdas/backup_system/backup_function.py"
  output_path = "${path.module}/src/lambdas/backup_system/backup_function.zip"
}
...
resource "aws_cloudwatch_event_target" "backup_lambda_target" {
  ...
  # Opcional: pasar evento personalizado
  input = jsonencode({
...
resource "aws_cloudwatch_metric_alarm" "backup_lambda_duration" {
  ...
  period              = 300
  statistic           = "Maximum"
  threshold           = var.lambda_timeout * 1000 * 0.9 # 90% del timeout
